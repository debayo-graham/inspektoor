...
class InspektoorSupabaseUser extends BaseAuthUser {
  InspektoorSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> inspektoorSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = InspektoorSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
